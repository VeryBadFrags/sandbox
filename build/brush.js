...
export default class Brush {
  ...
  init() {
    ...
    const spawnCell = (x, y) => {
      ...
      for (let i = Math.max(0, x - actualBrushSize); i < Math.min(x + actualBrushSize, canvasWidth); i++) {
        for (let j = Math.max(0, y - actualBrushSize); j < Math.min(y + actualBrushSize, canvasHeight); j++) {
    ...
    let prevMouseX = 0, prevMouseY = 0, mouseX = 0, mouseY = 0;
    ...
    function onMouseMove(e) {
      ...
      if (isMouseDown) {
        if (Utils.getDistance(mouseX, mouseY, prevMouseX, prevMouseY) > 2) {
          const interpolated = Utils.createIntermediatePoints(mouseX, mouseY, prevMouseX, prevMouseY);
